...
async def start(m):
    ...
    kb = build_buttons(s.get("welcome_buttons", ""))
    if photo:
        try:
...
async def router(m: Message, bot: Bot):
    ...
    if state == "welcome_text":
        db.set_setting("welcome_message", value); db.clear_state(uid)
        await m.answer("✅ Welcome text updated.", reply_markup=admin_kb())
    elif state == "verified_text":
        db.set_setting("verified_message", value); db.clear_state(uid)
        await m.answer("✅ Verified text updated.", reply_markup=admin_kb())
        ...
        try:
            cid = int(value.strip()); chat = await bot.get_chat(cid)
            db.add_channel(cid, chat.title or str(cid)); db.clear_state(uid)
            await m.answer("✅ Channel added.", reply_markup=admin_kb())
        except Exception as e: await m.answer(f"Could not access channel: {e}")
...
async def add_channel(c):
    if await auth_cb(c):
        db.set_state(c.from_user.id, "await_channel"); await c.message.answer("Send numeric channel ID, e.g. -1001234567890."); await c.answer()
...
async def join_request(req: ChatJoinRequest, bot: Bot):
    ...
    u=req.from_user
    db.record_join_request(u.id, req.chat.id, u.full_name)
    if db.is_verified(u.id):
        s=db.get_settings()
        ...
        try:
            if photo: await bot.send_photo(u.id, FSInputFile(photo), caption=text, reply_markup=kb)
            else: await bot.send_message(u.id, text, reply_markup=kb)
        except Exception: pass
        log.info("Verified request observed: %s in %s", u.id, req.chat.id)
    else:
        log.info("Unverified request observed: %s in %s", u.id, req.chat.id)

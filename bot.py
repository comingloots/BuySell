...
def channel_buttons():
    """Build Join Channel buttons from saved invite/request links only.

    Channel IDs are NOT required in the admin UI. The Telegram join request
    update itself provides the channel ID when a request arrives.
    """
    s = db.get_settings()
    ...
    for item in raw.split(";"):
        ...
        if "|" in item:
            ...
            label, url = "🔒 Join Channel", item

        if url:
            rows.append([InlineKeyboardButton(text=label or "🔒 Join Channel", url=url)])
...
async def router(m: Message, bot: Bot):
    ...
    if state == "welcome_text":
        ...
        links = value.strip()
        if not links:
            await m.answer("❌ Send at least one Request-to-Join link.")
            return
        db.set_setting("join_links", links)
        db.clear_state(uid)
        await m.answer(
            "✅ Join links saved.\n\n"
            "Ab /start par har saved private channel ka button dikhega.",
...
async def channels(c):
    if not await auth_cb(c):
        return
    s = db.get_settings()
    raw = s.get("join_links", "") or ""
    links = [x.strip() for x in raw.split(";") if x.strip()]
    text = (
        "📢 <b>Join Request Links</b>\n\n"
        + ("\n".join(f"• {escape(x)}" for x in links) if links else "No links configured.")
    )
    buttons = [
        [InlineKeyboardButton(text="🔗 Set Join Links", callback_data="add_channel")],
        [InlineKeyboardButton(text="⬅️ Back", callback_data="home")]
    ]
...
async def join_request(req: ChatJoinRequest, bot: Bot):
    ...
    # No manual channel-ID whitelist is required.
    # Telegram provides req.chat.id with every join-request update.
    log.info(
        "JOIN REQUEST RECEIVED chat_id=%s title=%r user_id=%s",
